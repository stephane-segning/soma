...
run-daemon:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-daemon -- --socket-path /tmp/soma-daemon.sock --db-path $SOMA_DATA_DIR/db/daemon.db --blob-dir $SOMA_DATA_DIR/blobs/daemon --listen-addrs /ip4/0.0.0.0/tcp/3000/ws

# Run the soma-agentd helper process
run-agentd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-agentd -- --socket-path /tmp/soma-agentd.sock --models-dir $SOMA_DATA_DIR/models --default-chat-model Meta-Llama-3-8B-Instruct.Q4_K_M.gguf
...
# Run soma-botd
run-botd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-botd -- --http-addr 127.0.0.1:0 --disable-mdns --listen-addrs /ip4/127.0.0.1/tcp/0

# Run soma-relayd
run-relayd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-relayd

# Run soma-rendezvousd
run-rendezvousd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-rendezvousd

# Run soma-bffd
run-bffd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-bffd

# Run soma-serverd (all-in-one server runner)
run-serverd:
	SOMA_DATA_DIR=$PWD/.data cargo run -p soma-serverd
