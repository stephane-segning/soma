...
Each cutover landed with a typecheck pass on both `pnpm --filter soma run typecheck:web` and (for backend-touching changes) `typecheck:node`. The pre-existing `pnpm --filter @soma/editor run typecheck` failure (`error TS2209: project root is ambiguous`) was fixed in [#83](https://github.com/stephane-segning/soma/pull/83) as part of the NodeAIRegistry-test PR — `tsconfig.json` gained `"rootDir": "./src"` and the spurious `@soma/ui/*` path alias was dropped. All four packages now typecheck cleanly on `main`.
...
- ~~**Capability record schema** — the `IssueIssuerCapability` daemon path stores peer id + expiry only. The Bots form captures `alias` + `scopeIds[]` and currently throws them away.~~ **Alias half landed in [#85](https://github.com/stephane-segning/soma/pull/85)** — new migration adds `issuer_capabilities.alias`, plumbed end-to-end through `IssueIssuerCapabilityInput` → napi → IPC → RTK Query → `useSpaceBots.addBot`. The list view shows the operator-typed alias instead of the synthesised `bot-<peerSuffix>` placeholder (which now only fires for legacy rows / blank inputs). **`scopeIds[]` storage half landed in [#92](https://github.com/stephane-segning/soma/pull/92)** — new migration adds `issuer_capabilities.scopes` (nullable TEXT, JSON-encoded array), plumbed end-to-end through storage → membership → daemon → napi → IPC → RTK Query → `useSpaceBots.addBot` (forwards `input.scopeIds` as `scopes`). **Runtime enforcement landed** — `membership::ensure_can_issue_membership` now reads `scopes` from the local row and rejects non-matching capabilities; empty scopes (`[]`) remain unrestricted for backward compat with pre-#92 rows. The only recognised scope in v0 is `"issue:membership"`. Scopes are enforced locally only (option B) — the signed proto does not yet carry a scopes field; that is a future hardening step (option A).
...
- ~~**`@mention` bots provider** — once `list_space_bots` ships, register a fourth `MentionProvider` with `section: "bots"` so `@bot:alias` resolves inline in the editor and chat composers.~~ **Landed in [#99](https://github.com/stephane-segning/soma/pull/99)** — new provider in `space-page/mentions.ts`. The trigger char is `!` (TipTap's Suggestion plugin is one-char-per-extension and `@` was already taken by members), so bots are addressed as `!bot-alias` in the editor — note the deliberate departure from the originally-planned `@bot:alias` syntax. The picker section was the existing `"bots"` kind added in cutover 5c. **Deep-link fix in [#101](https://github.com/stephane-segning/soma/pull/101)** — the original href pointed at a non-existent `/spaces/:id/settings/bots` route; replaced with `/spaces/:id/settings?tab=bots&peerId=...` and `space-settings.tsx` now reads the query string to pre-select the Bots tab + highlight the row.
- ~~**Block-level AI dispatch** — Cutover 5e didn't wire the action-menu's AI affordance. The plan is: click AI on the drag handle → set the block as a `NodeSelection` → `ContextualMenu` renders `SelectionAIBar` with `surface: "node"`. The registry and the UI both already understand the node surface; only the trigger is missing.~~ **Landed in [#95](https://github.com/stephane-segning/soma/pull/95)** — new `Star` AI button on the drag-handle menu, gated on `onQuickAction` being wired; click sets `NodeSelection` and opens a second `SelectionAIBar` instance anchored at the handle position. `createDefaultAIRegistry` actions now register on `["selection", "node"]` so the same catalog appears in both bars. Review caught a latent bug (selection-surface `action.run()` had never forwarded `metadata.from/.to`, so even the existing bubble flow was bailing out of `dispatchAndInsert`); fixed alongside.
- ~~**Editor-command icons** — `desktop/soma/.../space-page/editor-commands.ts` registers four slash commands (`new-sub-page`, `insert-image`, `insert-file`, `link-to-page`) without icons. SlashMenu now reserves the slot so alignment is fine, but the rows look bare next to the default commands. Rename to `.tsx` and pass `react-feather` icons.~~ **Landed in [#93](https://github.com/stephane-segning/soma/pull/93)** — `editor-commands.ts` → `.tsx` with `<File />`, `<Image />`, `<Paperclip />`, `<Link2 />` aliased to `FileIcon`/`ImageIcon` to avoid shadowing the DOM `File`/`Image` types (a real bug Gemini caught).
- ~~**Recent-pages slice** — `TreePopover` and `CommandPalette` both surface a Recent section that's empty in v0. Add a small redux slice keyed by `(spaceId, pageId, openedAt)`, capped at ~10 entries, hydrated from disk.~~ **Landed in [#100](https://github.com/stephane-segning/soma/pull/100)** — new `recentPages` slice + `recordPageOpened` action; `openedAt` flows in from the `useEffect` payload (not synthesised in the reducer, per review feedback) and persistence is a listener middleware (not a reducer side effect). `TreePopover` filters to the current space; `CommandPalette` shows the cross-space Recent list.
- **Default-expiry policy** — **landed (#97).** The daemon now enforces a 180-day maximum issuable lifetime (`MAX_ISSUER_CAPABILITY_LIFETIME_SECS` in `backend/crates/daemon/src/handle/issuer.rs`); the renderer's "Never" toggle still passes `0` and the daemon translates it to `now + 180 days` server-side. Explicit expiries above the ceiling are rejected with `expires_at exceeds maximum issuer capability lifetime`. Follow-up: surface "Expires in up to 180 days" near the toggle in `CapabilityForm` so operators aren't surprised by the bounded lifetime.
...
- ~~**`@soma/editor` typecheck broken on `main`** — `tsc --noEmit -p tsconfig.json` fails with `error TS2209: The project root is ambiguous`. Predates this revamp; surfaced because cutovers 5a–5e all touched the package. Workaround: typecheck via the soma renderer, which transitively type-checks the editor.~~ **Fixed in [#83](https://github.com/stephane-segning/soma/pull/83)** — `tsconfig.json` gained `"rootDir": "./src"` and dropped the `@soma/ui/*` path alias (which was forcing tsc to treat `desktop-ui/src/**` as in-package source). Workspace resolution at runtime is unchanged; the alias was only there for IDE jump-to-definition.
...
> **Update (final):** all four triage items above are now substantially addressed:
> 1. **IPC boundary contract tests** — [#82](https://github.com/stephane-segning/soma/pull/82) added `daemon-client.test.ts` (the test template that would have caught the cutover-1b unit-mismatch P0) + `spaces-service.integration.test.ts`. Subsequent PRs extended both for alias ([#85](https://github.com/stephane-segning/soma/pull/85)), status ([#86](https://github.com/stephane-segning/soma/pull/86)), `listSpaceBots` ([#84](https://github.com/stephane-segning/soma/pull/84)), and scopes ([#92](https://github.com/stephane-segning/soma/pull/92)).
> 2. **Hook tests for `use-space-bots`** — same PRs, plus the retry-button addition in [#90](https://github.com/stephane-segning/soma/pull/90).
> 3. **Registry-action tests for `createDefaultAIRegistry`** — landed in [#83](https://github.com/stephane-segning/soma/pull/83) (13 tests) and extended in [#95](https://github.com/stephane-segning/soma/pull/95) when block-level dispatch was added.
> 4. **Smoke tests for renderer wrappers** — [#91](https://github.com/stephane-segning/soma/pull/91) shipped 18 vitest+RTL render tests covering `SpacesRail`, `JumpToPageButton`, `CommandPaletteShell`, `BotsTab`. Implemented as vitest + jsdom (not Storybook stories) so they can mock the soma-specific RTK Query / i18n / IPC dependencies that don't have @soma/ui-side equivalents.
>
> CI gating: [#89](https://github.com/stephane-segning/soma/pull/89) added a `cargo build --workspace --locked` + `cargo test --workspace --locked` job to `.github/workflows/test.yml` (the napi-scoped `pnpm typecheck:node` was missing `somad`-binary build coverage, a gap that bit on #85). The repo now has Rust workspace + TS workspace + UI E2E gates on every PR.
