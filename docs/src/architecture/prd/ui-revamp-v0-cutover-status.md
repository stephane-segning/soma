...
- **`SomaHandle.list_space_bots`** — no napi method yet. The Bots tab's list view is empty until this lands. `useSpaceBots` is shaped to drop in a `useListSpaceBotsQuery` call without further component changes.
- **Capability record schema** — the `IssueIssuerCapability` daemon path stores peer id + expiry only. The Bots form captures `alias` + `scopeIds[]` and currently throws them away. Extending the capability record (or a sidecar bot-metadata store) is what makes those fields actually mean something.
