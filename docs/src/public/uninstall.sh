#!/bin/sh
# Soma uninstaller entry point.
#
# Tiny wrapper around bootstrap.sh — fetches and executes the uninstall.sh
# shipped with the latest (or pinned) desktop-v* GitHub Release.
#
# Usage:
#   curl -fsSL https://soma.camer.digital/uninstall.sh | sh
#   SOMA_PURGE_DATA=1 curl -fsSL https://soma.camer.digital/uninstall.sh | sh
...
if [ -n "${bootstrap_local:-}" ] && [ -f "$bootstrap_local" ]; then
  exec sh "$bootstrap_local" uninstall
fi
...
tmp="$(mktemp -d 2>/dev/null || mktemp -d -t soma-uninstall)"
trap 'rm -rf "$tmp"' EXIT INT HUP TERM

curl -fsSL "$bootstrap_url" -o "$tmp/bootstrap.sh"
exec sh "$tmp/bootstrap.sh" uninstall
