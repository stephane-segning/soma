...
- **Floating cards on a tinted shell.** The shell renders with `bg-base-200`; the rails (`ShellPanel` left/right) are transparent, and the surfaces *inside* the rails — the Pages sidebar on the left, each `Panel` inside `PanelContainer` on the right — are floating `bg-base-100` cards with a 1 px border + soft shadow + `p-2` gutter. Per-panel separation reads at a glance, which is the contract the user explicitly requested (a flush-hairline alternative was tried and reverted). The `ResizeHandle` between rails and main is itself a 1 px hairline whose colour shifts to the primary accent on hover — no widening pill, no transition-all.
...
- The right column is typically a `PanelContainer` — the 2-column stable-position layout for side panels (Chat / Bots / Page history / Agenda / …). Odd-index panels stack in column 1, even-index in column 2; each panel is a rounded `Panel` card sitting on the rail with a `p-2 gap-2` gutter, so the gray shell shows between cards. Collapsed panels show as icons in a `shrink-0` strip pinned to the right edge — when the rail is narrowed below the columns' natural width, the *columns* scroll horizontally (`min-w-0 overflow-x-auto`) so the strip stays visible, never disappearing off the edge.
