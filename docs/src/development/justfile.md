...
Note: CI-critical automation uses `cargo xtask` for Cargo workspace tasks and `@soma/packaging` for bundle packaging (see `docs/src/development/xtask.md`). The `justfile` remains shorthand for local development and light CI aggregation.

## Preferred grouped targets

- **Backend builds / runs** – `just backend-build-daemons`, `just backend-run-daemon`, `just backend-run-agentd`, `just backend-build-servers`, `just backend-run-botd`, `just backend-run-relayd`, `just backend-run-rendezvousd`, `just backend-run-bffd`, `just backend-run-serverd`
- **Backend tests / xtask** – `just backend-test`, `just backend-test-relayd-smoke`, `just backend-test-rendezvousd-smoke`, `just backend-xtask-help`, `just backend-xtask-version-workspace`
- **Desktop workspace** – `just desktop-install`, `just desktop-run-soma`, `just desktop-build-soma`, `just desktop-test-soma`, `just desktop-test-tapia`, `just desktop-test-all`
- **Desktop icons** – `just desktop-icons-soma`, `just desktop-icons-tapia`
...
- **Electron Tapia (`desktop/tapia`)** – `just desktop-icons-tapia /path/to/icon.png` (or `.svg`)
