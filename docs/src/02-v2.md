...
- a single Electron desktop app under `desktop/soma` (with the merged-in
...
- `soma-daemon` (library only): peer identity, SQLite, blobs, spaces, pages,
  documents, join flows. Linked into the `@soma/node` napi addon and run
  in-process inside Electron main; no standalone binary.
- `soma-agentd` (library only): Yjs merge/drift handling and the agent
  runtime. Same packaging story as `soma-daemon`.
...
Tapia is no longer a separate Electron app — it has been merged into Soma as the `/practice` route. Typing practice is pure-renderer (no daemon dependency); the shared backend-integrated surfaces (spaces, pages, blobs) live in the main Soma routes.
...
- `proto/daemon/v1/daemon.proto` (kept as a record-shape reference for the
  in-process napi surface; no longer a gRPC service)
- `proto/agent/v1/agent.proto` (same — in-process napi only)
- daemon event stream semantics (delivered via napi `ThreadsafeFunction`,
  not a Unix-socket gRPC stream)
...
- shared `proto/` source used by both Rust and TypeScript (now mostly for
  record types; the desktop side calls the napi addon directly, not gRPC)
- napi-addon ABI between `@soma/node` and the desktop's main process
- stage naming assumptions shared across repos
- daemon event stream semantics consumed by desktop over napi
  `ThreadsafeFunction`
