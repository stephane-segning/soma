...
const api = createApi({
	...
	endpoints: (builder) => ({
		...
		setSetting: builder.mutation<void, { key: string; value: unknown }>({
		...
		updatePageTitle: builder.mutation<
			PageRecord | null,
			{ spaceId: string; pageId: string; title: string }
		>({
			queryFn: async (input) => {
				try {
		...
		setPageParents: builder.mutation<
			PageRecord | null,
			{ spaceId: string; pageId: string; parentPageIds: string[] }
		>({
			queryFn: async (input) => {
				try {
		...
		>({
			...
			providesTags: (_result, _error, { spaceId, documentId }) => [
				{ type: "Draft", id: `${spaceId}:${documentId}` },
			],
		}),
		...
		listAgentModels: builder.query<chatService.AgentModel[], void>({
			...
			providesTags: () => [{ type: "AgentModels", id: "LIST" }],
		}),

		streamChat: builder.mutation<
			null,
			{ history: chatService.ChatMessage[]; model?: string }
		>({
