...
import { Calendar, Clock, Cpu, FileText, Hash, List, MessageSquare, Plus, Search } from "react-feather";
...
export function SomaAppRender() {
	...
	return (
		<DesktopShell
			...
			leftColumn={
				leftHasExpanded ? (
					<PanelContainer expandedIds={leftExpanded} onCollapse={collapseLeft} panels={LEFT_PANELS} />
				) : null
			}
			mainClassName="bg-base-100"
			mainTopLeft={
				<PanelChipBar expandedIds={leftExpanded} onToggle={toggleLeft} panels={LEFT_PANELS} placement="top-left" />
			}
			mainTopRight={
				<PanelChipBar expandedIds={rightExpanded} onToggle={toggleRight} panels={RIGHT_PANELS} placement="top-right" />
			}
			rightColumn={
				rightHasExpanded ? (
					<PanelContainer expandedIds={rightExpanded} onCollapse={collapseRight} panels={RIGHT_PANELS} />
				) : null
			}
		>
	...
	return (
		<div className="flex items-center justify-between px-2 py-1">
			...
			<div className="flex items-center gap-1">
				...
				<BackendSwitcher activeId={activeBackend} backends={BACKENDS} onChange={onChangeBackend} />
			</div>
		</div>
	);
}

function PagesPanel({ activeId, onSelect }: { activeId: string; onSelect: (id: string) => void }) {
			...
			{PAGES.map((page) => (
				<div
					...
					className={`cursor-pointer list-row hover:bg-base-200 ${
						page.id === activeId ? "bg-base-200 font-medium text-base-content" : ""
					}`}
					key={page.id}
		...
		<ul className="list list-dense bg-base-100">
			{headings.map((h, idx) => (
				<li className="list-row hover:bg-base-200" key={`${h.level}-${idx}`}>
					<span className="text-[10px] text-base-content/40" style={{ paddingInlineStart: (h.level - 1) * 12 }}>
						H{h.level}
					</span>
...
function EditorMock({ title }: { title: string }) {
	return (
		<div className="mx-auto max-w-3xl px-8 py-10">
			...
			<p className="mb-6 text-base-content/60 text-xs">Last edited 3 min ago by you · saved locally</p>
			<p className="mb-4 text-sm leading-relaxed">
				This is a styled mock of the document surface. The real renderer drops
				<span className="mx-1 rounded bg-base-200 px-1.5 py-0.5 font-mono text-xs">@soma/editor</span>
				here, with the full Tiptap stack: slash menu, drag handle, format bubble, and language-aware code blocks. Visit
				the <span className="text-primary">Editor → DocumentEditor</span> story for the live version.
			</p>
			<p className="mb-6 text-sm leading-relaxed">
				The floating chip bars at the top-left and top-right of this column control the side rails. Click an icon to
				expand a panel into its rail (the rail will appear with its persisted width); click the <Kbd size="xs">−</Kbd>{" "}
				on a panel header to collapse it back into the chip bar.
			</p>
			...
			<ul className="mb-6 space-y-2 text-sm">
				<li className="flex items-start gap-2">
					...
					<span className="text-base-content/60 line-through">Pin slash menu to the `/` trigger</span>
				</li>
				<li className="flex items-start gap-2">
					<input checked className="checkbox checkbox-xs mt-0.5" readOnly type="checkbox" />
					<span className="text-base-content/60 line-through">Wire UndoRedo so ⌘Z works</span>
				</li>
				<li className="flex items-start gap-2">
		...
		<div className="flex h-full flex-col">
			<div className="flex-1 space-y-2 overflow-auto p-2 text-[13px]">
				<div className="space-y-0.5">
					<span className="font-medium text-[11px] text-base-content/60">You · 2m ago</span>
					<p className="text-base-content/80">Summarize this page in three bullets.</p>
				</div>
				<div className="space-y-0.5">
					<span className="font-medium text-[11px] text-primary">Assistant</span>
					<p className="text-base-content/80">
						Editor polish PR ships Cmd+K link insertion, the bright drop cursor, and a unified MenuShell primitive
						across slash / context / AI bar.
					</p>
				</div>
			</div>
			<div className="border-base-300 border-t p-2">
				<input className="input input-sm w-full" placeholder="Ask anything…" type="text" />
			</div>
		</div>
	);
}
