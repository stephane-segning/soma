...
describe("SecretInput", () => {
	it("renders masked (type=password) by default", () => {
		renderWithIntl(
			<SecretInput onChange={() => undefined} value="sk-live-secret" />,
		);
	...
	it("calls onChange with the typed value, not a masked placeholder", () => {
		...
		const { container } = renderWithIntl(
			<SecretInput onChange={onChange} value="" />,
		);
	...
	it("calls onBlur when the field loses focus", () => {
		const onBlur = vi.fn();
		const { container } = renderWithIntl(
			<SecretInput onBlur={onBlur} onChange={() => undefined} value="" />,
		);
		const input = container.querySelector("input") as HTMLInputElement;
	...
	it("renders the caller-supplied placeholder (used to signal a stored-but-unshown value)", () => {
		renderWithIntl(
			<SecretInput
				onChange={() => undefined}
				placeholder="Unchanged — leave blank to keep the current key"
				value=""
			/>,
		);
		...
		expect(
			screen.getByPlaceholderText(
				"Unchanged — leave blank to keep the current key",
			),
		).not.toBeNull();
	});
	...
	it("renders the label when provided and associates it with the input", () => {
		renderWithIntl(
			<SecretInput label="API key" onChange={() => undefined} value="" />,
		);
