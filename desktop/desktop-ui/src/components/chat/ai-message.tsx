		...
		<div className={cn("flex gap-2", className)}>
			...
			<div className="flex-1 space-y-2">
				<div className="flex items-center gap-2">
					...
					{message.meta ? <span className="text-base-content/60 text-xs">{message.meta}</span> : null}
				</div>
				{message.thinking ? <AiThinking {...message.thinking} className="border border-base-300/60" /> : null}
				{message.content ? <AiMarkdown className="prose prose-sm max-w-none" content={message.content} /> : null}
			</div>
		</div>
	);
}
