...
export function Modal({ open, title, description, children, actions, onClose, placement = "center" }: ModalProps) {
	useHotkeys(
		"esc",
		(event) => {
				...
				{open ? (
					...
					>
						<motion.div
							...
							className={cn("glass-panel shadow-elevated", bottom ? "w-full p-4" : "max-w-xl p-6")}
