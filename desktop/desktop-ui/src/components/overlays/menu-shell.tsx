 ...
 */
export const MenuShell = forwardRef<HTMLDivElement, MenuShellProps>(function MenuShell(
	{ className, width = "min-w-48", role = "menu", ...rest },
	ref,
) {
	return (
		<div
			className={cn("glass-panel flex flex-col gap-0.5 p-1 shadow-elevated", width, className)}
			ref={ref}
			role={role}
			{...rest}
		/>
...
export const MenuItem = forwardRef<HTMLButtonElement, MenuItemProps>(function MenuItem(
	{ label, icon, shortcut, tone = "default", active = false, className, disabled, role, ...rest },
	ref,
) {
	...
	const rowChildren = (
		<>
			{icon != null ? (
				<span aria-hidden className="inline-flex size-4 shrink-0 items-center justify-center text-base-content/60">
					{icon}
				</span>
			) : null}
...
export function MenuSectionLabel({ children }: { children: ReactNode }) {
	return <div className="px-2 pt-1 text-base-content/50 text-xs uppercase tracking-wide">{children}</div>;
}
