...
pub(super) fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/v1/spaces_list_my_memberships",
            post(spaces_list_my_memberships),
        )
        ...
        .route(
            "/api/v1/spaces_list_join_requests",
            post(spaces_list_join_requests),
        )
        .route("/api/v1/spaces_revoke_member", post(spaces_revoke_member))
        .route("/api/v1/spaces_revoke_bot", post(spaces_revoke_bot))
        .route(
...
async fn spaces_delete(
    ...
    spaces::delete(&app, body.space_id)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
...
async fn spaces_list_members(
    ...
    spaces::list_members(&app, body.space_id)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
...
async fn spaces_list_my_memberships(
    ...
    spaces::list_my_memberships(&app)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
...
async fn spaces_list_bots(
    ...
    spaces::list_bots(&app, body.space_id)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
...
async fn spaces_revoke_member(
    ...
    spaces::revoke_member(&app, args)
        .await
        .map(Json)
        .map_err(ApiError::from)
}

async fn spaces_revoke_bot(
    State(app): State<Arc<AppState>>,
    Json(args): Json<spaces::RevokeBotArgs>,
) -> Result<Json<bool>, ApiError> {
    spaces::revoke_bot(&app, args)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
...
async fn spaces_issue_issuer_capability(
    ...
    spaces::issue_issuer_capability(&app, args)
        .await
        .map(Json)
        .map_err(ApiError::from)
}
