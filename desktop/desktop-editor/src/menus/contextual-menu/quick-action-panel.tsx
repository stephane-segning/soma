...
export function QuickActionPanel({ resultText, resultTone, runningAction, selection, onRun }: QuickActionPanelProps) {
	return (
		...
		>
			...
			{!resultText ? <QuickActionList onRun={onRun} runningAction={runningAction} /> : null}
			{resultText ? (
				<div
					className={cn(
						"mt-3 max-h-64 overflow-y-auto whitespace-pre-wrap rounded-lg border px-3 py-2 text-sm",
						resultTone === "error" ? "border-error/50 bg-error/10 text-error-content" : "border-base-300 bg-base-200",
					)}
				>
					{resultText}
				</div>
...
function QuickActionList({
	...
	return (
		<ul className="mt-3 divide-y divide-base-300 overflow-hidden rounded-lg border border-base-300">
			<QuickActionButton
				action="explain"
				icon={<HelpCircle className="size-4 text-base-content/70" />}
			...
			<QuickActionButton
				action="expand"
				icon={<Maximize2 className="size-4 text-base-content/70" />}
			...
			<QuickActionButton
				action="research"
				icon={<Search className="size-4 text-base-content/70" />}
