...
export function registerExerciseIpc(): void {
	ipcMain.handle(
		...
		(_event, spaceId: string) => exercisesBySpace.get(spaceId) ?? [],
	);
	...
	ipcMain.handle(
		...
		(_event, attempt: ExerciseAttempt) => ({
			ok: true,
			leaderboard: recordAttempt(attempt),
		}),
	);
	ipcMain.handle(
...
function storeExercise(draft: ExerciseDraft): Exercise {
	...
	exercisesBySpace.set(meta.spaceId, [
		exercise,
		...(exercisesBySpace.get(meta.spaceId) ?? []),
	]);
	return exercise;
}

function recordAttempt(attempt: ExerciseAttempt): LeaderboardEntry[] {
	attemptsBySpace.set(attempt.spaceId, [
		...(attemptsBySpace.get(attempt.spaceId) ?? []),
		attempt,
	]);
