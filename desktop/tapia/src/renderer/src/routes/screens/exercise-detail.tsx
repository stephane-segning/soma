...
function ExerciseDetail(): React.JSX.Element {
  ...
  const { exercises, status, recordSession, findExercise } = useExercises(activeSpaceId)

  const exercise = useMemo(
    () => findExercise(params.exerciseId) ?? exercises.find((item) => item.meta.id === params.exerciseId),
  ...
  if (!exercise) {
    return (
      <section className="panel">
        ...
        <Link className="ghost-button" to={`/spaces/${params.spaceId}/exercises`}>
          {t('exercises')}
        </Link>
      </section>
    )
    ...
    <section className="panel">
      <div className="panel__head">
        <div>
          ...
          <p className="muted">
            {exercise.meta.length} {t('characters')} · {exercise.meta.difficulty}
          </p>
        </div>
        <Link className="ghost-button" to={`/spaces/${exercise.meta.spaceId}/exercises`}>
          {t('exercises')}
        </Link>
      </div>
