	...
	parsers: {
			...
			deserialize: {
				...
				parse: (el, editor) => {
					if (el.nodeName === "IMG") {
						...
						const maxSizes = (
							editor.plugins.Image.options as ImagePluginOptions
						)?.maxSizes;
						...
						const props: SlateElement<"image", ImageElementProps>["props"] = {
							...
							fit: (el.getAttribute("objectFit") ||
								"contain") as ImageElementProps["fit"],
							sizes: limitedSizes,
						};
		...
		markdown: {
			serialize: (element, text) =>
				`![${element.props.alt || element.id}](${element.props.src})\n`,
		},
		email: null,
	},
});
