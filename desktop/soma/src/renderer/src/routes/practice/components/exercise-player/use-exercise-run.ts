...
export function useExerciseRun({ exercise, onComplete }: UseExerciseRunInput) {
	...
	const correctCharacters = useMemo(() => countCorrectCharacters(exercise.message, input), [exercise.message, input]);
	...
	const wpm =
		elapsedMs > 0 && startedAt ? Math.round((correctCharacters / 5 / (elapsedMs / 1000)) * 60 * 100) / 100 : 0;
