		...
		<div className="card border border-base-300 bg-base-100">
			<div className="card-body space-y-4">
				...
				<div className="flex items-center gap-2">
					...
					/>
					<button className="btn btn-sm" onClick={props.addCapabilityModel} type="button">
						Add
					</button>
				</div>
				<div className="flex items-center justify-end gap-2">
					<button className="btn btn-outline btn-sm" onClick={props.normalizeCapabilities} type="button">
						Normalize
					</button>
					<button
	...
	return useMemo<ColumnDef<GlobalCapabilityRow>[]>(
		() => [
			...
			...CAPABILITY_COLUMNS.map(
				({ key, label }): ColumnDef<GlobalCapabilityRow> => ({
					header: label,
					cell: ({ row }) => (
						<input
