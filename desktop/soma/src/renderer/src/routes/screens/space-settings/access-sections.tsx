...
export function SpaceAccessSummary({
	...
	return (
		<div className="grid gap-3 md:grid-cols-3">
			<SummaryCard label="Members" title={String(memberRows.length)} detail={membershipSummary(memberRows)} />
			<SummaryCard label="Pending requests" title={String(pendingJoinRequests.length)} detail="Approve or reject people waiting for access" />
			<div className="rounded-xl border border-base-300 bg-base-100 px-4 py-3">
				...
				<div className="mt-2">
					<Link className="btn btn-ghost btn-xs" to={`/spaces/${spaceId}/members`}>Open members view</Link>
				</div>
			</div>
		</div>
	);
		...
		<div className="card border border-base-300 bg-base-100">
			<div className="card-body space-y-3">
				...
				<div className="rounded-xl border border-base-300 bg-base-200/50 px-4 py-3 text-xs text-base-content/70">
					...
					<div className="mt-2">Bot membership does not automatically grant join approval authority. Bot actions should stay within approved automation for this space.</div>
				</div>
				<TanstackTable
