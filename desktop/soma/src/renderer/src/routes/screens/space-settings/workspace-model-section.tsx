	...
	return (
		<>
			<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
				<div className="rounded-xl border border-base-300 bg-base-100 px-4 py-3 text-base-content/70 text-sm md:col-span-2">
			...
			<div className="card border border-base-300 bg-base-100">
				<div className="card-body">
					...
					<div className="flex items-center gap-2 pt-2">
						...
						/>
						<button className="btn btn-sm" onClick={props.onAddCapabilityModel} type="button">
							Add
						</button>
					</div>
					<div className="flex items-center justify-end gap-2 pt-2">
						<button className="btn btn-outline btn-sm" onClick={() => props.setWorkspaceDraft({})} type="button">
							Clear workspace overrides
						</button>
						<button
							className="btn btn-primary btn-sm"
							disabled={!props.spaceId || props.isSaving}
...
function useWorkspaceCapabilityColumns(
	...
	return useMemo<ColumnDef<WorkspaceCapabilityRow>[]>(
		() => [
			...
			...CAPABILITY_COLUMNS.map(
				({ key, label }): ColumnDef<WorkspaceCapabilityRow> => ({
					header: label,
					cell: ({ row }) => (
						<input
