...
const documentsSlice = createSlice({
	...
	reducers: {
			...
			state,
			action: PayloadAction<{ spaceId: string; documentId: string }>,
		) {
		...
		setContentJson: {
			prepare(spaceId: string, documentId: string, contentJson: string) {
				return {
					payload: { spaceId, documentId, contentJson, editedAtMs: Date.now() },
				};
			},
			reducer(
				state,
				...
				state.byKey[key] = {
					...
					lastEditedAtMs: action.payload.editedAtMs,
				};
			},
		},
		setPublished(
		...
		hydrateFromDraft(state, action: PayloadAction<DraftDocument>) {
...
const documentsSelectors = {
	selectByKey: (state: { documents: DocumentsState }) => state.documents.byKey,
};

export { documentsActions, documentsReducer, documentsSelectors, makeKey };
export type { DocumentEditorState, DocumentKey, DocumentsState, DraftDocument };
