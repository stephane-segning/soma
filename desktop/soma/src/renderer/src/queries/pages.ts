		...
		useCallback(
			async (
				...
				if (
					...
					navigate(
						`/spaces/${spaceId}/pages/${created.pageId}`,
					);
				}
			},
			[
...
function useUpdatePageTitleMutation() {
	const [
		...
		api.useUpdatePageTitleMutation();
	return {
		...state,
		isPending:
			state.isLoading,
