	...
	register(
		...
		ipc.handle(
			...
			(
				...
				this.documents.queueDaemonSync(
					params,
				),
		);
		ipc.handle(
		...
		ipc.handle(
			...
			(
				...
				this.documents.updatePageTitle(
					params,
				),
		);
		ipc.handle(
		...
		ipc.handle(
			...
			(
				...
				this.agent.resolveDrift(
					{
						leftUpdateBase64:
							params?.leftUpdateBase64 ??
							params?.left_update_base64 ??
							"",
						rightUpdateBase64:
							params?.rightUpdateBase64 ??
							params?.right_update_base64 ??
							"",
					},
				),
		);
		...
		ipc.handle(
			...
			(
				...
				switch (
					...
					case "minimize":
						return this.windows.minimize(
							window,
						);
					case "toggleMaximize":
						return this.windows.toggleMaximize(
							window,
						);
					case "close":
