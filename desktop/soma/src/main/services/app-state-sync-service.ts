...
export class AppStateSyncService {
  ...
  constructor(
    @inject(TYPES.appSettingsService) private readonly settings: AppSettingsService,
    ...
    const windowEvents$ = merge(
      ...
      tap(() => {
        ...
        this.settings
          ...
          .catch((error) => this.logger.warn('Failed to persist window bounds', error))
      })
    )
  ...
  private trackRendererMessages(): void {
    ...
    const kvSet$ = this.ipc
      .onRendererEvent<KvSetPayload>(CHANNELS.kvSet)
      .pipe(
