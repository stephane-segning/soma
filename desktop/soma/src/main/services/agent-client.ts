...
export class AgentClient {
	...
	async chatStream(
		...
		try {
			...
			let combined =
				"";
			return await new Promise<StreamEvent>(
				(
					resolve,
					reject,
				) => {
	...
	> {
		try {
			const res =
				await new Promise<ListModelsResponse>(
					(
						resolve,
						reject,
					) => {
	...
	async resolveDrift(
		...
		if (
			!params.leftUpdateBase64?.trim()
		) {
			throw new Error(
				"leftUpdateBase64 is required",
			);
		}
		if (
			!params.rightUpdateBase64?.trim()
		) {
			throw new Error(
				"rightUpdateBase64 is required",
			);
		}
