...
export class DaemonClient {
	...
	async uploadBlob(
		...
		const res =
			await new Promise<UploadBlobResponse>(
				(
					resolve,
					reject,
				) => {
	...
	async readBlob(
		...
		try {
			const res =
				await new Promise<ReadBlobResponse>(
					(
						resolve,
						reject,
					) => {
	...
	async getDocument(
		...
		try {
			const res =
				await new Promise<GetDocumentResponse>(
					(
						resolve,
						reject,
					) => {
	...
	async ensurePage(
		...
		if (
			...
			throw new Error(
				"Daemon returned empty page",
			);
		return this.fromPageRecord(
			p,
		);
	}
	...
	> {
		const res =
			await new Promise<ListPagesResponse>(
				(
					resolve,
					reject,
				) => {
	...
	async listSpaces(options?: {
		...
		const res =
			await new Promise<ListSpacesResponse>(
				(
					resolve,
					reject,
				) => {
